...
const userSchema = new mongoose.Schema(
  {
    ...
    canvasFingerprint: { type: String, default: "", index: true },
    webglFingerprint: { type: String, default: "", index: true },
...
const channelJoinLogSchema = new mongoose.Schema(
  {
...
channelJoinLogSchema.index({ chatId: 1, telegramId: 1 }, { unique: true });
const ChannelJoinLog = mongoose.models.ChannelJoinLog || mongoose.model("ChannelJoinLog", channelJoinLogSchema);

// Cumulative join counter — once a user is logged as joined for a channel,
// they stay counted even if they leave later (upsert is a no-op on repeat).
async function recordChannelJoin(chatId, telegramId) {
  try {
    await ChannelJoinLog.updateOne(
      { chatId: String(chatId), telegramId: String(telegramId) },
      { $setOnInsert: { chatId: String(chatId), telegramId: String(telegramId) } },
      { upsert: true }
    );
  } catch (err) {
    // duplicate key races are fine to ignore — it just means it was already logged
  }
}
  ...
  for (const ch of forcedChannels) {
    ...
    try {
      ...
      if (["left", "kicked"].includes(member.status)) {
        ...
        recordChannelJoin(ch.chatId, userId); // fire-and-forget — cumulative stat, never decrements
      }
    } catch (err) {
      pending.push({ name: ch.name, url: ch.url });
    }
...
function checkDevVerifyAdmin(req, res, next) {
  const pass = req.headers["x-admin-password"] || req.body.adminPassword || req.query.adminPassword;
  if (pass !== DEV_VERIFY_PASSWORD && pass !== ADMIN_PASSWORD) return res.status(401).json({ error: "Unauthorized" });
  next();
}
...
async function handleChatJoinRequest(joinReq) {
  try {
    ...
    recordChannelJoin(chatId, telegramId); // fire-and-forget — counts toward the channel's join stat
  } catch (err) {
...
app.post("/api/verify", async (req, res) => {
  const { telegramId, passed, fingerprint, canvasFingerprint, webglFingerprint, chatId, messageId } = req.body;
  const user = await User.findOne({ telegramId });
  if (!user) return res.status(404).json({ error: "User not found" });
  ...
  if (canvasFingerprint) user.canvasFingerprint = canvasFingerprint;
  if (webglFingerprint) user.webglFingerprint = webglFingerprint;
  if (ip) {
    user.lastIp = ip;
  ...
  if (!passed) {
    ...
    if (referrer) {
      const sameDeviceAsReferrer =
        (fingerprint && referrer.deviceFingerprint && referrer.deviceFingerprint === fingerprint) ||
        (canvasFingerprint && referrer.canvasFingerprint && referrer.canvasFingerprint === canvasFingerprint) ||
        (webglFingerprint && referrer.webglFingerprint && referrer.webglFingerprint === webglFingerprint);
    ...
    if (!banReason) {
      ...
      if (canvasFingerprint) orConditions.push({ canvasFingerprint: canvasFingerprint });
      if (webglFingerprint) orConditions.push({ webglFingerprint: webglFingerprint });
...
// Cumulative join count per channel — counts joins/join-requests we've ever
// observed for that chat. Never decreases when someone leaves later.
app.get("/api/admin/channel-stats", checkAdmin, async (req, res) => {
  const settings = await Settings.getSettings();
  const channels = [];
  let totalMembersJoin = 0;

  for (const ch of settings.channels) {
    const totalJoin = await ChannelJoinLog.countDocuments({ chatId: String(ch.chatId) });
    totalMembersJoin += totalJoin;
    channels.push({ channelUsername: ch.name, chatId: ch.chatId, link: ch.url, totalJoin });
  }

  res.json({ channels, totalMembersJoin });
});

// Scans every added channel/group/private chat and checks whether the bot
// is still an admin there. Reports anything that's broken and pings the
// bot admin(s) in chat too.
app.post("/api/admin/check-bot-admin", checkAdmin, async (req, res) => {
  const settings = await Settings.getSettings();
  const missingAdmin = [];

  for (const ch of settings.channels) {
    const isAdmin = await verifyBotIsAdmin(ch.chatId);
    if (!isAdmin) missingAdmin.push({ name: ch.name, chatId: ch.chatId });
  }

  if (missingAdmin.length > 0) {
    const list = missingAdmin.map((c) => `• ${c.name} (\`${c.chatId}\`)`).join("\n");
    const text = `⚠️ *Bot Admin Check*\n\nBot is NO LONGER admin in:\n${list}\n\nForce-join / broadcast will not work correctly for these until the bot is re-added as admin.`;
    for (const adminId of ADMIN_TELEGRAM_IDS) {
      await bot.sendMessage(adminId, text, { parse_mode: "Markdown" }).catch((err) => console.error("admin-check notify failed:", err.message));
    }
  }

  res.json({ checked: settings.channels.length, missingAdmin, allGood: missingAdmin.length === 0 });
});
