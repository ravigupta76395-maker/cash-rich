...
async function handleEntry(chatId, telegramId) {
  ...
  if (user && user.banned) {
    const pending = await getForcedPendingChannels(telegramId, settings.channels);
    await sendAccessRestricted(chatId, pending.length ? pending : settings.channels.filter((c) => c.forced !== false));
    return;
  }
  ...
  if (pendingChannels.length > 0) {
    ...
    await sendAccessRestricted(chatId, pendingChannels);
    return;
  }
